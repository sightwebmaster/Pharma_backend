...
public class MedicamentController {
     ...
     * Retourne le catalogue complet pour recommendation-service.
     */
    @GetMapping
    public ResponseEntity<List<MedicamentResponse>> getAll() {
        log.info("Récupération du catalogue complet des médicaments");
        return ResponseEntity.ok(medicamentService.getAll());
    }

    @GetMapping("/catalog")
    public ResponseEntity<List<MedicamentResponse>> getCatalog() {
        log.info("Récupération du catalog endpoint pour recommendation-service");
        return ResponseEntity.ok(medicamentService.getCatalog());
    }

    @GetMapping("/by-disease/{disease}")
    public ResponseEntity<List<MedicamentResponse>> getByDisease(
            @PathVariable String disease) {
        log.info("Recherche médicaments par maladie={}", disease);
        return ResponseEntity.ok(medicamentService.getByDisease(disease));
    }
